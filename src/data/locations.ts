export const regions = [
    "Ahafo",
    "Ashanti",
    "Bono",
    "Bono East",
    "Central",
    "Eastern",
    "Greater Accra",
    "North East",
    "Northern",
    "Oti",
    "Savannah",
    "Upper East",
    "Upper West",
    "Volta",
    "Western",
    "Western North",
];

export const cities: Record<string, string[]> = {
    "Greater Accra": [
        "Abeka / Abeka-Lapaz",
        "Abelemkpe",
        "Ablekuma",
        "Abokobi",
        "Abossey Okai",
        "Accra Central / High Street",
        "Accra Mall / Tetteh Quarshie",
        "Accra New Town",
        "Achimota",
        "Adabraka",
        "Adenta",
        "Airport Residential Area / Airport City",
        "Akweteyman",
        "Alajo",
        "Amasaman",
        "Anyaa",
        "Ashaiman",
        "Ashaley Botwe",
        "Ashongman Estate",
        "Asylum Down",
        "Atomic",
        "Awoshie",
        "Awudome",
        "Baatsona (Spintex)",
        "Bubuashie",
        "Burma Camp",
        "Cantonments",
        "Chorkor",
        "Circle (Kwame Nkrumah Circle)",
        "Dansoman",
        "Darkuman",
        "Dome",
        "Dzorwulu",
        "East Legon / East Legon Hills",
        "Gbawe",
        "Haatso",
        "Kanda",
        "Kaneshie",
        "Kasoa",
        "Kissieman",
        "Kokomlemle",
        "Korle Bu / Korle Gonno",
        "Kwabenya",
        "Kwashieman",
        "Labadi (La)",
        "Labone",
        "Lapaz",
        "Lashibi",
        "Laterbiokorshie",
        "Legon",
        "Madina",
        "Mallam",
        "Mamobi",
        "Mamprobi",
        "Mataheko",
        "Nima",
        "Nungua",
        "Odorkor",
        "Ofankor",
        "Osu",
        "Oyarifa",
        "Pantang",
        "Pokuase",
        "Roman Ridge",
        "Sakumono",
        "Santa Maria",
        "Sowutuom",
        "Spintex Road",
        "Taifa",
        "Tema (Communities 1–25)",
        "Teshie / Teshie-Nungua",
        "Tesano",
        "Tse Addo",
        "Ussher Town",
        "Weija",
        "West Legon (Westland)",
    ],
    // Other regions can be populated as needed, default to empty or generic
    "Ashanti": ["Kumasi", "Obuasi", "Mampong", "Ejura"],
    "Central": ["Cape Coast", "Winneba", "Kasoa", "Swedru"],
    "Eastern": ["Koforidua", "Nkawkaw", "Nsawam", "Akim Oda"],
    "Western": ["Takoradi", "Sekondi", "Tarkwa", "Axim"],
    "Volta": ["Ho", "Hohoe", "Aflao", "Keta"],
    "Northern": ["Tamale", "Yendi", "Savelugu"],
    "Upper East": ["Bolgatanga", "Navrongo", "Bawku"],
    "Upper West": ["Wa", "Jirapa", "Tumu"],
    "Bono": ["Sunyani", "Berekum", "Dormaa Ahenkro"],
    "Bono East": ["Techiman", "Kintampo", "Atebubu"],
    "Ahafo": ["Goaso", "Duayaw Nkwanta", "Bechem"],
    "Savannah": ["Damongo", "Salaga", "Bole"],
    "North East": ["Nalerigu", "Walewale", "Gambaga"],
    "Oti": ["Dambai", "Jasikan", "Kadjebi"],
    "Western North": ["Sefwi Wiawso", "Bibiani", "Enchi"]
};

